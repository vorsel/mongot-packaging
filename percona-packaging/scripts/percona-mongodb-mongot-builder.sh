...
Usage: $0 [OPTIONS]
    The following options may be given :
        ...
        --help) usage ;;
Example $0 --builddir=/tmp/percona-mongodb-mongot --get_sources=1 --build_mongot=1 --build_variant=linux-x64
EOF
        exit 1
}
...
get_sources(){
    ...
    PRODUCT=percona-mongodb-mongot
    echo "PRODUCT=${PRODUCT}" > percona-mongodb-mongot.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> percona-mongodb-mongot.properties
    echo "BUILD_ID=${BUILD_ID}" >> percona-mongodb-mongot.properties
    echo "VERSION=${VERSION}" >> percona-mongodb-mongot.properties
    echo "BRANCH=${BRANCH}" >> percona-mongodb-mongot.properties
    rm -rf ${PRODUCT}
    ...
    echo "export REVISION=${REVISION}" >> ${WORKDIR}/percona-mongodb-mongot.properties
    cd ${WORKDIR}
    ...
    # mongot itself ships no native packaging — copy ours in so the source
    # tarball is self-contained for both rpmbuild and dpkg-buildpackage.
    if [ ! -d ${CURDIR}/percona-packaging ]; then
        echo "ERROR: ${CURDIR}/percona-packaging not found."
        exit 1
    fi
    cp -r ${CURDIR}/percona-packaging ${PRODUCT}-${VERSION}/percona-packaging
    tar --owner=0 --group=0 --exclude=.git -czf ${PRODUCT}-${VERSION}.tar.gz ${PRODUCT}-${VERSION}
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT}-${VERSION}/${BRANCH}/${REVISION}/${BUILD_ID}" >> percona-mongodb-mongot.properties
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      yum -y install wget git tar gzip bzip2 which curl unzip zip make gcc gcc-c++ python3 python3-pip rpm-build rpmdevtools systemd-rpm-macros
      ...
      if [ "x${RHEL}" = "x7" ]; then
          ...
          # Bazel/Bazelisk needs a recent toolchain even though mongot bundles its own JDK at runtime.
          yum -y install java-11-openjdk-devel devtoolset-11
      ...
      INSTALL_LIST="wget git devscripts debhelper debconf pkg-config make gcc g++ python3 python3-pip tar gzip bzip2 curl unzip zip ca-certificates fakeroot lintian"
      case "${DEBIAN}" in
          buster|bullseye)
              INSTALL_LIST="${INSTALL_LIST} openjdk-17-jdk-headless"
              ;;
          *)
              INSTALL_LIST="${INSTALL_LIST} openjdk-21-jdk-headless"
              ;;
      esac
...
get_tar(){
    TARBALL=$1
    TARFILE=$(basename $(find $WORKDIR/$TARBALL -name 'percona-mongodb-mongot*.tar.gz' | sort | tail -n1))
    if [ -z $TARFILE ]
    then
        TARFILE=$(basename $(find $CURDIR/$TARBALL -name 'percona-mongodb-mongot*.tar.gz' | sort | tail -n1))
        if [ -z $TARFILE ]
        then
...
build_mongot(){
    ...
    TARFILE=$(basename $(find . -name 'percona-mongodb-mongot*.tar.gz' | sort | tail -n1))
    tar xzf ${TARFILE}
    cd ${PRODUCT}-${VERSION}
    source VERSION
...
build_rpm(){
    ...
    TARFILE=$(basename $(find . -maxdepth 1 -name 'percona-mongodb-mongot*.tar.gz' | sort | tail -n1))
    # Pull percona-packaging/ and VERSION out of the source tarball so we can
    # render the spec template before invoking rpmbuild.
    ...
    sed -e "s:@@VERSION@@:${VERSION}:g" \
        ...
        percona-packaging/rpm/percona-mongodb-mongot.spec > rpmbuild/SPECS/percona-mongodb-mongot.spec
    cp ${TARFILE} rpmbuild/SOURCES/
    cp mongot-community-bundle.tar.gz rpmbuild/SOURCES/

    echo "RHEL=${RHEL}" >> ${WORKDIR}/percona-mongodb-mongot.properties
    echo "ARCH=${ARCH}" >> ${WORKDIR}/percona-mongodb-mongot.properties

    rpmbuild -bb \
        --define "_topdir ${WORKDIR}/rpmbuild" \
        --define "dist .${OS_NAME}" \
        --define "version ${VERSION}" \
        rpmbuild/SPECS/percona-mongodb-mongot.spec
    return_code=$?
...
build_deb(){
    ...
    echo "DEBIAN=${DEBIAN}" >> ${WORKDIR}/percona-mongodb-mongot.properties
    echo "ARCH=${ARCH}"   >> ${WORKDIR}/percona-mongodb-mongot.properties
...
CURDIR=$(pwd)
VERSION_FILE=$CURDIR/percona-mongodb-mongot.properties
args=
WORKDIR=
SRPM=0
SDEB=0
...
REVISION=0
BRANCH="main"
REPO="https://github.com/vorsel/mongot.git"
PRODUCT=percona-mongodb-mongot
VARIANT="linux-x64"
